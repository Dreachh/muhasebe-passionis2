...
export const fetchExchangeRates = async (): Promise<any> => {
  try {
    ...
    const rates = [
      ...
      { code: "USD", name: "Amerikan Doları", buying: fallbackRates.USD, selling: Number((fallbackRates.USD * 1.02).toFixed(2)) },
      { code: "EUR", name: "Euro", buying: fallbackRates.EUR, selling: Number((fallbackRates.EUR * 1.02).toFixed(2)) },
      { code: "GBP", name: "İngiliz Sterlini", buying: fallbackRates.GBP, selling: Number((fallbackRates.GBP * 1.02).toFixed(2)) },
      { code: "SAR", name: "Suudi Arabistan Riyali", buying: fallbackRates.SAR, selling: Number((fallbackRates.SAR * 1.02).toFixed(2)) },
    ];
    return {
      rates,
