...
export function FirebaseAuthProvider({ 
  ...
  // Email/Şifre ile giriş - sadece belirli kullanıcılar için
  const signIn = async (email: string, password: string): Promise<User | null> => {
    try {
      setLoading(true);
      setError(null);
      
      // E-posta kontrolü önce yapılır - izin verilen bir e-posta adresi mi?
      const { isAllowedEmail, isAllowedUser } = await import('./auth-config');
      ...
      // Önce Firebase'e normal giriş yapalım
      const userCredential = await signInWithEmailAndPassword(auth, email, password);
      const user = userCredential.user;
      
      // Kullanıcı ID kontrolü - eğer yeni ise izin verilenlere ekleme ihtiyacı olabilir
      // (Yeni kullanıcı için UID'yi kontrol etme)
      if (!isAllowedUser(user.uid)) {
        console.log("Yeni kullanıcı ID'si:", user.uid);
        
        // İzin verilen e-posta adresine dayanarak izin ver
        if (user.email && isAllowedEmail(user.email)) {
          // İzin verilen kullanıcı
          console.log("İzin verilen e-posta adresiyle giriş yapıldı:", user.email);
          setUser(user);
          return user;
        } else {
          ...
          await signOut(auth);
          setError("Bu hesap ile giriş yapma yetkisine sahip değilsiniz. Sadece yetkili hesaplar kullanılabilir.");
          setUser(null);
          return null;
        }
      }
