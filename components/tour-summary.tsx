            ...
            <Table>
              ...
              <TableBody>
                {activities.map((activity, idx) => {
                  const katilimci = activity.participants || '-';
                  const birimUcret = activity.price ? `${activity.price} ${activity.currency || ''}` : '-';
                  const toplamUcret = activity.price && activity.participants ? `${Number(activity.price) * Number(activity.participants)} ${activity.currency || ''}` : '-';
                  return (
                    <TableRow key={idx} className="h-8">
