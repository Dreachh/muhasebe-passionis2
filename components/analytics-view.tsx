...
export function AnalyticsView({ financialData = [], toursData = [], onClose }) {
       ...
       tour.expenses.forEach((expense) => {
         if (expense && expense.amount) {
           allExpenses.push({
             ...
             currency: expense.currency || tour.currency || "TRY",
             category: expense.category || "Tur Gideri",
             date: expense.date || tour.tourDate,
           });
         }
       });
     }
   });
   // Toplam gider (tüm para birimleri karışık)
   const totalExpense = allExpenses.reduce((sum, item) => sum + (Number.parseFloat(item?.amount?.toString() || '0') || 0), 0)
  ...
  // Gider kategorileri
   ...
   const expenseByCategory = allExpenses.reduce((acc, item) => {
     const category = item.category || "Diğer"
     if (!acc[category]) {
       acc[category] = 0
     }
