...
interface Expense {
  ...
  acentaName?: string;
  category?: string; // HATA: category alanı eklendi
  expenseTypeId?: string; // HATA: expenseTypeId alanı eklendi
  companyId?: string; // Firma seçimi için eklendi
}
...
export function TourSalesForm({
  ...
  useEffect(() => {
    // Adım 2'de sadece kişi başı fiyat * kişi sayısı hesaplanır (aktiviteler dahil edilmeden)
    const basePrice = Number(formData.pricePerPerson || 0) * Number(formData.numberOfPeople || 1);
    
    // Adım 2'de tur toplam fiyatında aktivite toplamını dahil etmiyoruz
    // Sadece kişi sayısı x kişi başı fiyatını gösteriyoruz
    const total = basePrice;
    console.log(`Tur fiyatı hesaplanıyor: Temel fiyat=${basePrice}, Toplam=${total} ${formData.currency}`);
  ...
  function handleSubmit() {
    if (typeof onSave === "function") {
      ...
      try {
        // Form durumunu temizle - Formu kaydettiğimizde işlemin tamamlandığını belirtmek için tüm verileri temizleme
        localStorage.removeItem(TOUR_SALES_FORM_KEY);
        localStorage.removeItem(TOUR_SALES_STEP_KEY);
        localStorage.removeItem(TOUR_IN_PROGRESS_KEY); // Form işlem durumunu temizle
      ...
      if (typeof onCancel === "function") {
        ...
        }, 500); // Kullanıcının bildirimi görmesi için kısa bir gecikme ekle
      ...
      try {
        ...
        if (cachedCompanies) {
          ...
          if (Array.isArray(parsedCompanies) && parsedCompanies.length > 0) {
            console.log("Firmalar önbellekten yüklendi:", parsedCompanies.length, "adet");
  ...
  useEffect(() => {
    // localStorage olmadığında veya düzenleme modundaysa kaydetme
    if (typeof localStorage === 'undefined' || initialData) {
      return;
    }
  ...
  }, [formData, currentStep, initialData, toast]);
  ...
  }, [initialData, tempTourFormData, steps.length]);
  ...
  const handleCancel = () => {
    try {
      ...
      localStorage.removeItem(TOUR_SALES_STEP_KEY);
      localStorage.removeItem(TOUR_IN_PROGRESS_KEY);
      console.log("Form durumu temizlendi, iptal edildi");
    } catch (error) {
      console.error("Form durumu temizlenirken hata:", error);
    }
